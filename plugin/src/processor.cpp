...
juce::StringArray NeuralAmpProcessor::getSortedNamModelNames(const juce::File& namFolder, std::vector<juce::String>& modelPaths)
{
    ...
    {
        DBG("Error: NAM directory does not exist or is not accessible: " << namFolder.getFullPathName());
        return modelNames;
    }
...
juce::AudioProcessorValueTreeState::ParameterLayout NeuralAmpProcessor::createParameterLayout()
{
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>("inputLevel", "Input Level", juce::NormalisableRange<float>(-20.0f, 20.0f, 0.1f), 0.0f));
    layout.add(std::make_unique<juce::AudioParameterFloat>("toneBass", "Bass", juce::NormalisableRange<float>(0.0f, 10.0f, 0.1f), 5.0f));
    layout.add(std::make_unique<juce::AudioParameterFloat>("toneMid", "Middle", juce::NormalisableRange<float>(0.0f, 10.0f, 0.1f), 5.0f));
    layout.add(std::make_unique<juce::AudioParameterFloat>("toneTreble", "Treble", juce::NormalisableRange<float>(0.0f, 10.0f, 0.1f), 5.0f));
    layout.add(std::make_unique<juce::AudioParameterFloat>("outputLevel", "Output Level", juce::NormalisableRange<float>(-40.0f, 40.0f, 0.1f), 0.0f));
    layout.add(std::make_unique<juce::AudioParameterBool>("noiseGateActive", "Noise Gate Active", true));
    layout.add(std::make_unique<juce::AudioParameterFloat>("noiseGateThreshold", "Noise Gate Threshold", juce::NormalisableRange<float>(-100.0f, 0.0f, 0.1f), -80.0f));
    ...
    layout.add(std::make_unique<juce::AudioParameterFloat>("inputCalibrationLevel", "Input Calibration Level", juce::NormalisableRange<float>(-60.0f, 60.0f, 0.1f), 12.0f));
    ...
    layout.add(std::make_unique<juce::AudioParameterChoice>("selectedNamModel", "NAM Model", choices, 0));
...
void NeuralAmpProcessor::setCurrentModelIndex(int index)
{
    ...
    if (index >= 0 && index < static_cast<int>(modelPathsByIndex.size()) && modelPathsByIndex[static_cast<size_t>(index)].isNotEmpty())
    {
...
void NeuralAmpProcessor::addModel(const juce::String& filePath)
{
    ...
    const juce::ScopedLock lock(modelLoadLock);
    auto* param = parameters.getParameter("selectedNamModel");
    int maxModels = param ? dynamic_cast<juce::AudioParameterChoice*>(param)->choices.size() : modelNames.size();
...
void NeuralAmpProcessor::loadNamFromFile(const juce::String& filePath)
{
    ...
    try
    {
        ...
        if (rawDsp)
        {
            ...
            std::shared_ptr<nam::DSP> sharedDsp = std::move(rawDsp);
            dsp.store(sharedDsp);
